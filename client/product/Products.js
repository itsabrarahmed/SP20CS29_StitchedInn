  ...
  const classes = useStyles()
    return (
      ...
      {props.products.length > 0 ?
          ...
          {props.products.map((product, i) => (
            <GridListTile key={i} className={classes.tile}>
              ...
              <GridListTileBar className={classes.tileBar}
                ...
                subtitle={<span>PKR {product.price}</span>}
                // actionIcon={
        ...
        </GridList></div>) : props.searched && (<Typography variant="subheading" component="h4" className={classes.title}>No Designs found! :(</Typography>)}
      </div>)
}
Products.propTypes = {
