...
export default function AllTailors() {
    ...
    let [status, setStatus] = useState(true);
      const handleChange = (index,id) => {
        ...
        setStatus(!status);
          ...
          const user = {
            status:!tailorsTableData[index].status
          }
        update({
            userId: id
          }, {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
       ...
       .then(res=>res.map((element)=>{
         ...
         if(element.seller==true && element.email.split("@")[1] !== "admin.com"){
    ...
    return (
      ...
      <Table className={classes.table} aria-label="simple table">
        ...
        <TableBody>
          {tailorsTableData.map((row,index) => (
            <TableRow key={index}>
